...
def convert_srtm_to_xyz(latitude, longitude, srtm_folder, output_file,
                         del_angle=1.5, max_points=25000):
    """
    Read SRTM raster tiles covering the area around *latitude* / *longitude*,
    crop to a bounding box of ±del_angle degrees, and write a space-separated
    text file of ``longitude  latitude  elevation`` (one point per line).

    This is the exact equivalent of the original R script's output file
    ...
    # Write output (space-separated, no header, matching R's write.table)
